...
/*
* ViewModel for the SearchItemsFragment screen.
* The ViewModel works with the ItemsRepository to get the data.
* */
@HiltViewModel
class SearchItemsFragmentViewModel @Inject constructor(private val itemsRepository: ItemsRepository) : ViewModel() {

    /* Search items whose names match the given query */
    fun searchItems(query: String) : Flow<PagingData<Item>> =
        itemsRepository
            /*
            * The procedure of searching items is delegated to the searchItems() function in ItemsRepository.
